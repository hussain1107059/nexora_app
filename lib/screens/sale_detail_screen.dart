  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _infoCard('Sale Date', _formatDate(sale.date)),
            const SizedBox(height: 12),
            _infoCard('Sale Type', sale.saleType),
            const SizedBox(height: 12),
            _infoCard('Customer', sale.customer),
            if (sale.chalanNo.isNotEmpty) ...[
              const SizedBox(height: 12),
              _infoCard('Chalan No', sale.chalanNo),
            ],
            if (sale.partyNo.isNotEmpty) ...[
              const SizedBox(height: 12),
              _infoCard('Party No', sale.partyNo),
            ],
            const SizedBox(height: 12),
            _infoCard('Serial No', sale.serialNo),
            const SizedBox(height: 12),
            _infoCard('Truck', sale.truck),
            if (sale.remarks.isNotEmpty) ...[
              const SizedBox(height: 12),
              _infoCard('Remarks', sale.remarks),
            ],
            const SizedBox(height: 12),
            _buildItemsSection(),
            const SizedBox(height: 12),
            _infoCard('Total Amount', '\$${sale.totalAmount.toStringAsFixed(2)}'),
          ],
        ),
      ),
    );
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  sale.items.map((e) => e.warehouse).toSet().join(', '),
